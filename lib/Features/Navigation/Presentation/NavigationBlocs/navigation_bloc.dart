...
class WishBloc extends Bloc<WishesEvent, WishesState> {
  ...
  Future<void> _makeWish(MakeWish event, Emitter<WishesState> emit) async {
    ...
    res.fold(
      (l) => emit(FailureState(message: l)),
      (r) => emit(SuccessState(message: r)),
    );
  }

  Future<FutureOr<void>> _unWish(
    DeleteWish event,
    Emitter<WishesState> emit,
  ) async {
  ...
  Future<FutureOr<void>> _getWishes(
    WishesExtracted event,
    Emitter<WishesState> emit,
  ) async {
    ...
    res.fold(
      (l) => emit(FailureState(message: l)),
      (r) => emit(WishesExtractedState(wishes: r)),
    );
  }

  Future<FutureOr<void>> _getWishedProducts(
    GetWishedProducts event,
    Emitter<WishesState> emit,
  ) async {
    emit(WishLoadingState());
    ...
    res.fold(
      (l) => emit(FailureState(message: l)),
      (r) => emit(WishedProductsExtractedState(products: r)),
    );
  }

  Future<FutureOr<void>> _getItemsCount(
    GetItemsCount event,
    Emitter<WishesState> emit,
  ) async {
