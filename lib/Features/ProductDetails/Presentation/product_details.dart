import 'package:ecommerce_store/Core/DeviceUtils/device_utils.dart';
import 'package:ecommerce_store/Core/Widgets/brand_chip_with_image.dart';
import 'package:ecommerce_store/Core/Widgets/circular_containers.dart';
import 'package:ecommerce_store/Core/Widgets/curved_edges.dart';
import 'package:ecommerce_store/Core/Widgets/my_rectangular_image.dart';
import 'package:ecommerce_store/Core/Widgets/my_snack_bar.dart';
import 'package:ecommerce_store/Core/formatter/color_formats.dart';
import 'package:ecommerce_store/Features/Navigation/Controllers/wishlist_controller.dart';
import 'package:ecommerce_store/Features/ProductDetails/Presentation/Controllers/product_details_controller.dart';
import 'package:ecommerce_store/Features/ProductDetails/Presentation/reviews.dart';
import 'package:ecommerce_store/Features/Settings/Presentaion/settingBloc/settings_bloc.dart';
import 'package:ecommerce_store/Features/Settings/domain/Entities/cart_entity.dart';
import 'package:ecommerce_store/Features/Widgets/see_picture.dart';
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  DiscountRow(price: details.price, off: details.off),
                  ...
                  const SizedBox(height: Sizes.spaceBtwItems),
                  const MyRichText(leading: 'Stock: ', trailing: 'In Stock'),
                  const SizedBox(height: Sizes.spaceBtwItems),

                  BrandChipWithImage(
                    /*logo: BrandController.brands[0].image,*/ name:
                        details.brand,
                    isDark: isDark,
                    width: Sizes.buttonWidth * 4,
                  ),
                  const SizedBox(height: Sizes.spaceBtwSections),
                  Variation(isDark: isDark, price: details.price),
                  const SizedBox(height: Sizes.spaceBtwItems),
                  Text(
                    'Colors: ',
            ...
            children: [
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: () {
                    if (widget.controller.selected == -1 ||
                        widget.controller.selectedColor == -1) {
                      mySnackBar(
                        'Please Select at least one Color or size',
                        context,
                      );
                    } else {
                      context.read<SettingsBloc>().add(
                        AddToCartEvent(
                          item: CartItem(
                            ...
                            quantity:
                                widget.controller.quantity > 0
                                    ? widget.controller.quantity
                                    : 1,
                            size: widget.controller.selectedSize,
                            color: widget.controller.selectedColorName,
                            price:
                                widget.details.price -
                                (widget.details.price *
                                    (double.parse(widget.details.off!) /
                                        100.0)),
...
class Variation extends StatelessWidget {
  const Variation({super.key, required this.isDark, required this.price});
  final double price;
  final bool isDark;
  @override
  Widget build(BuildContext context) {
      ...
      child: Row(
        children: [
          ...
          Column(
            ...
            children: [
              MyRichText(leading: 'Price: ', trailing: '\$$price'),
              const MyRichText(leading: 'Stock: ', trailing: 'In Stock'),
            ],
          ),
        ],
        ...
        child: Column(
          children: [
                  ...
                  child: IconButton(
                    onPressed: () {
                      if (!WishListController.checkWish(widget.productId)) {
                        ...
                        context.read<WishBloc>().add(
                          DeleteWish(id: widget.productId),
                        );
            ...
            GestureDetector(
              onTap:
                  () => Get.to(
                    () => ViewImage(imageUrl: widget.images[selected]),
                  ),
...
class DiscountRow extends StatelessWidget {
  const DiscountRow({super.key, required this.price, this.off});
  final double price;
  final String? off;
  @override
  Widget build(BuildContext context) {
          ...
          Container(
            ...
            child: Center(
              child: Text(
                ...
                style: Theme.of(
                  context,
                ).textTheme.labelSmall!.apply(color: Colors.black),
              ),
            ),
          ),
...
class MyProductRatingAndShare extends StatelessWidget {
  const MyProductRatingAndShare({super.key});
    ...
    return Row(
      children: [
        SizedBox(
          ...
          child: Row(
            children: [
              const Icon(Iconsax.star5, size: Sizes.lg, color: Colors.yellow),
              const SizedBox(width: Sizes.sm),
              Text('4.2(199)', style: Theme.of(context).textTheme.bodyLarge),
            ],
          ),
        ),
